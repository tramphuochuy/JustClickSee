...
//			_registryKey.SetValue("Order.BltCopyBits.Enabled", 1);
        ...
        {
            if (_bitmapHeight != Height || Width != _bitmapWidth)
            {
                try { Disconnect(); }
                catch (Exception ex) { }
                try { Unload(); }
                ...
                try
                {
                    ...
                    if (State == MirrSharp.Driver.DesktopMirror.MirrorState.Loaded)
                    {
                        Connect();
                    }

                    if (State == MirrSharp.Driver.DesktopMirror.MirrorState.Connected)
                    {
                        Start();
                    }
                }
